...
mod tests
{
	...
	fn display()
	{
		...
		assert_str_eq!(
			Contact { kind: ContactKind::Email("foo@bar.io".into()), label: "Email".into() }.to_string(),
			"Email: foo@bar.io"
		);
		assert_str_eq!(
			Contact { kind: ContactKind::Phone("1-603-555-5555".into()), label: "Cellphone".into() }.to_string(),
